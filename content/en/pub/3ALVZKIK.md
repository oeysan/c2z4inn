---
title: Doing research into Indigenous issues being non-Indigenous
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-3ALVZKIK" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Skille, E. Å. (2021). Doing research into Indigenous issues being non-Indigenous. <i>Qualitative Research</i>, 14–1. <a href="https://doi.org/10.1177/14687941211005947">https://doi.org/10.1177/14687941211005947</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-3ALVZKIK" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=1907813" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/3ALVZKIK" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-3ALVZKIK" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-3ALVZKIK" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-3ALVZKIK" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-3ALVZKIK" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://journals.sagepub.com/doi/pdf/10.1177/14687941211005947" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://journals.sagepub.com/doi/pdf/10.1177/14687941211005947" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-3ALVZKIK"></div>
</article>
<div id="csl-bib-meta-3ALVZKIK" class="csl-bib-meta">
  <article id="about-article-3ALVZKIK" class="about_pub-article">
    <h1>About</h1>
    Skille explores the unique challenges and opportunities faced by non-Indigenous researchers studying Indigenous issues, specifically through the lens of Sámi sport. They emphasize the importance of critical self-reflection, ensuring that research benefits Indigenous communities, and maintaining respectful dialogue. Skille also highlights the diversity within Indigenous groups, noting that researchers must be aware of varying perspectives and experiences. This study underscores the need for researchers to be conscious of their own biases and the dynamic relationship between themselves and the Indigenous communities they study. These insights are crucial for conducting respectful and meaningful research that supports and uplifts Indigenous voices.
  </article>
  <article id="keywords-article-3ALVZKIK" class="keywords-article">
    <h1>Keywords</h1>
    Indigenous Issues, Sámi Sport, Non-Indigenous Researcher, Critical Self-Reflection, Reciprocity, Heterogeneity
  </article>
  <article id="abstract-article-3ALVZKIK" class="abstract-article">
    <h1>Scientific abstract</h1>
    Based on research into Indigenous people and sport, this article discusses the opportunities and 
challenges for a non-Indigenous researcher to study Indigenous issues. The author shares personal 
experiences from research into Sámi sport (Sámi are the Indigenous people of the North Calotte) 
and compares these with the literature relating to post-colonial methodologies. It concludes with 
some overarching elements to take into consideration when researching Indigenous peoples: 
reflection including critical self-reflection (as in any qualitative research); reciprocity, including 
respect, dialogue and that the research must benefit the Indigenous people under study and in 
general; and awareness about the heterogeneity within Indigenous groups and consciousness 
about the interface between researcher and Indigenous peoples. Given the heterogeneity, the 
interfaces vary
  </article>
  <article id="contributors-article-3ALVZKIK" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Eivind Åsrum Skille" loading="lazy"> </div> <div class="info"> <hgroup><h1>Eivind Åsrum Skille</h1> <h2>Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:eivind.skille@inn.no">eivind.skille@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762430054"> +47 62 43 00 54 </a></dd> </dl> <p> Faculty of Social and Health Sciences<br> Section for Sports and Physical Education<br> Campus Elverum, Room 4R4142 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.88156,11.53723">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328312" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-3ALVZKIK" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg16" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg16_en.png" class="image" alt="SDG 16">
        <div class="sdg-overlay">
          <a href="{{< params subfolder >}}en/archive/?sdg=16#archive" class="sdg-publication-count"><span>479</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal16" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-3ALVZKIK" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li><a href="{{< params subfolder >}}en/archive/?key=3DCRN523">University of Inland Norway</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=IDKFS3MX">Faculty of Social and Health Sciences</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=FJXE3Z8X">Department of Public Health and Sport Sciences</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=HKMXV8PC">2021</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=UJSVBC5D">May</a></li>
    </ul>
  </article>
</div>
