---
title: Anything still goes! jakta på eit paradigme etter den fleirspråklege vendinga
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-6GL9LAGG" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Randen, G. T. (2022). Anything still goes! jakta på eit paradigme etter den fleirspråklege vendinga. In S. Ragnhildstveit, A. Golden, &#38; A.-K. H. Gujord (Eds.), <i>Nye innsikter i norsk som andrespråk: inspirert av Kari Tenfjords forskning</i> (pp. 263–282). Alvheim &#38; Eide akademisk forlag.</div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-6GL9LAGG" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2113762" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/6GL9LAGG" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-6GL9LAGG" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-6GL9LAGG" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-6GL9LAGG" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-6GL9LAGG" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
  </div>
  <div id="csl-bib-meta-container-6GL9LAGG"></div>
</article>
<div id="csl-bib-meta-6GL9LAGG" class="csl-bib-meta">
  <article id="about-article-6GL9LAGG" class="about_pub-article">
    <h1>About</h1>
    Randen and colleagues have compiled a collection of articles inspired by Kari Tenfjord's research on learning Norwegian as a second language. Tenfjord, a former professor at the University of Bergen, was a pioneer in this field. The anthology explores various aspects of second language acquisition, including how learners transfer knowledge from their first language, the use of language corpora (large collections of texts), teaching methods, and the overall study of second languages. This work is valuable for researchers, teachers, and students who are interested in these topics, as it builds on Tenfjord's influential contributions and offers new insights into effective language learning strategies.
  </article>
  <article id="keywords-article-6GL9LAGG" class="keywords-article">
    <h1>Keywords</h1>
    Transfer, Corpus, Didactics, Second Language Education
  </article>
  <article id="abstract-article-6GL9LAGG" class="abstract-article">
    <h1>Scientific abstract</h1>
    Kari Tenfjord var professor i norsk som andrespråk ved Universitetet i Bergen og en av banebryterne innenfor fagfeltet andrespråksopplæring. Denne antologien består av artikler som på ulike vis bygger på, og videreutvikler, Kari Tenfjords forskningsinteresser innenfor temaene transfer, korpus, didaktikk og andrespråksfaget som helhet. Artiklene er av interesse for forskere, lærere og studenter som er opptatt av og arbeider med disse temaene.
  </article>
  <article id="contributors-article-6GL9LAGG" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Gunhild Tveit Randen" loading="lazy"> </div> <div class="info"> <hgroup><h1>Gunhild Tveit Randen</h1> <h2>Associate Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:gunhild.randen@inn.no">gunhild.randen@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762517887"> +47 62 51 78 87 </a></dd> <dt>Mobile number</dt> <dd><a href="tel:+4740853629"> +47 40 85 36 29 </a></dd> </dl> <p> Faculty of education<br> Department of Scandinavian Languages and Literature<br> Campus Hamar, Room 2C256-4 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.79625,11.07386">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/gunhild-randen.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/gunhild-randen.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328387" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-6GL9LAGG" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg4" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg04_en.png" class="image" alt="SDG 4">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=4#archive" class="sdg-publication-count"><span>1352</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal4" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-6GL9LAGG" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=WYNZA47F">Faculty of Education</a>
      </li>
      <li>
        <a href="/en/archive/?key=T9U6ILTU">Department of Scandinavian Languages and Literature</a>
      </li>
      <li>
        <a href="/en/archive/?key=2WDT9FBV">2023</a>
      </li>
      <li>
        <a href="/en/archive/?key=S822ZERV">January</a>
      </li>
    </ul>
  </article>
</div>
