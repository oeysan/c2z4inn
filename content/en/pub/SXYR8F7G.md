---
title: 'The art of balance: Indigenous sport governance between traditional government
  and self-governance'
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-SXYR8F7G" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Lehtonen, K., Skille, E. Å., &#38; Fahlen, J. (2023). The art of balance: Indigenous sport governance between traditional government and self-governance. <i>International Review for the Sociology of Sport</i>, 19. <a href="https://doi.org/10.1177/10126902231217185">https://doi.org/10.1177/10126902231217185</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-SXYR8F7G" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2215618" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/SXYR8F7G" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-SXYR8F7G" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-SXYR8F7G" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-SXYR8F7G" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-SXYR8F7G" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://journals.sagepub.com/doi/pdf/10.1177/10126902231217185" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://journals.sagepub.com/doi/pdf/10.1177/10126902231217185" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-SXYR8F7G"></div>
</article>
<div id="csl-bib-meta-SXYR8F7G" class="csl-bib-meta">
  <article id="about-article-SXYR8F7G" class="about_pub-article">
    <h1>About</h1>
    Lehtonen et al. examined how Sámi sport governance in Finland balances traditional practices with self-governance ambitions. They found that this balance often leads to contradictions and exclusionary processes. The study focused on strategies used to legitimize Sámi sport policies and governance. They identified three main strategies: authorization, which involves gaining approval from institutions and individuals; moral evaluation, based on the personal choices of influential figures; and inclusion and integration into mainstream policies, supported by narratives emphasizing the uniqueness and small size of the Sámi community. These findings are important because they highlight the complexities and challenges Indigenous groups face in managing their sports and cultural activities while striving for self-determination and broader political influence.
  </article>
  <article id="keywords-article-SXYR8F7G" class="keywords-article">
    <h1>Keywords</h1>
    Indigenous Governance, Sámi Sport, Self-Governance, Political Influence, Legitimation Strategies
  </article>
  <article id="abstract-article-SXYR8F7G" class="abstract-article">
    <h1>Scientific abstract</h1>
    The governance of Indigenous people is in many contexts a combination of political ambitions to promote self-governance, and more traditional policies and governance practices. These combinations often carry unintended contradictions and exclusionary processes. In this article, we investigate the consequences of one such contradiction: the aspiration for self-determination and self-governance on the one hand and the aspiration for broader political influence in decisions about resources to Sámi sport on the other. Since legitimation of governance structures and practices is essential for their overall functionality, we constructed the research question: What strategies are used to legitimise the policy and governance practices of Sámi sport? To explore this research question, we employed Sámi sport in Finland as an empirical case. Results show that authorisation as a legitimation strategy is prominent and used at institutional and individual levels. Moral evaluation as strategy is based on authoritative actors’ personal choice. Inclusion and integration in mainstream policy is seen as a rational legitimation strategy, which is supported by narratives where smallness and uniqueness are dominant.
  </article>
  <article id="contributors-article-SXYR8F7G" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Eivind Åsrum Skille" loading="lazy"> </div> <div class="info"> <hgroup><h1>Eivind Åsrum Skille</h1> <h2>Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:eivind.skille@inn.no">eivind.skille@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762430054"> +47 62 43 00 54 </a></dd> </dl> <p> Faculty of Social and Health Sciences<br> Section for Sports and Physical Education<br> Campus Elverum, Room 4R4142 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.88156,11.53723">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328312" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-SXYR8F7G" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg10" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg10_en.png" class="image" alt="SDG 10">
        <div class="sdg-overlay">
          <a href="{{< params subfolder >}}en/archive/?sdg=10#archive" class="sdg-publication-count"><span>404</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal10" class="sdg-read-more">Read More</a></p>
        </div>
      </div> <div id="sdg16" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg16_en.png" class="image" alt="SDG 16">
        <div class="sdg-overlay">
          <a href="{{< params subfolder >}}en/archive/?sdg=16#archive" class="sdg-publication-count"><span>479</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal16" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-SXYR8F7G" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li><a href="{{< params subfolder >}}en/archive/?key=3DCRN523">University of Inland Norway</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=IDKFS3MX">Faculty of Social and Health Sciences</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=FJXE3Z8X">Department of Public Health and Sport Sciences</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=5HKEZMYN">2023</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=CKXFWIRH">December</a></li>
    </ul>
  </article>
</div>
