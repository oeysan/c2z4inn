---
title: Arbetsförhållanden och välbefinnande i arbetet inom den svenska Försvarsmakten
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-MPJQXKPI" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Österberg, J., &#38; Rydstedt, L. W. (2024). Arbetsförhållanden och välbefinnande i arbetet inom den svenska Försvarsmakten. <i>Arbetsmarknad &#38; Arbetsliv</i>, <i>30</i>(1), 25–39. <a href="https://doi.org/10.58236/aa.25473">https://doi.org/10.58236/aa.25473</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-MPJQXKPI" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2261014" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/MPJQXKPI" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-MPJQXKPI" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-MPJQXKPI" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-MPJQXKPI" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-MPJQXKPI" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://journals.lub.lu.se/aoa/article/download/25473/22970" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://journals.lub.lu.se/aoa/article/download/25473/22970" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-MPJQXKPI"></div>
</article>
<div id="csl-bib-meta-MPJQXKPI" class="csl-bib-meta">
  <article id="about-article-MPJQXKPI" class="about_pub-article">
    <h1>About</h1>
    Österberg and Rydstedt conducted a survey study to understand how work demands, support, resources, and conflicts between work and personal life affect the well-being of Swedish military personnel. They surveyed 1002 officers and specialist officers, mostly men, with an average age of 41. Using statistical analysis, they found that high work demands were strongly linked to lower well-being at work. Conversely, having support and resources from the organization and immediate supervisors was strongly linked to higher well-being. Additionally, conflicts between work and personal life significantly reduced reported well-being. These findings highlight the importance of managing work demands and providing adequate support to improve the well-being of military personnel.
  </article>
  <article id="keywords-article-MPJQXKPI" class="keywords-article">
    <h1>Keywords</h1>
    Work Conditions, Well-being, Swedish Military, Work-Life Conflict, Organizational Support
  </article>
  <article id="abstract-article-MPJQXKPI" class="abstract-article">
    <h1>Scientific abstract</h1>
    Syftet med denna survey-studie med tvärsnitts-design var att belysa hur arbetskrav, 
stöd och resurser, samt konflikter mellan arbete – övrigt liv relaterade till 
välbefinnande i arbetet för svenska miliärer. Undersökningsgruppen bestod 
av 1002 svenska anställda officerare och specialistofficerare (ålder m=41; 90 
procent män/ 10 procent kvinnor). För den statistiska analysen användes 
hierarkisk multipel regressionsanalys. Efter kontroll för demografiska bakgrundsfaktorer 
introducerades arbetsinnehållet, där upplevt höga arbetskrav 
relaterade starkt negativt - medan resurser och stöd från organisationen/ 
närmaste chef relaterade starkt positivt – till arbetsrelaterat välbefinnande. 
Slutligen introducerades konflikter mellan arbete och övrigt liv, vilket 
relaterade signifikant negativt till rapporterat arbetsrelaterat välbefinnande.
  </article>
  <article id="contributors-article-MPJQXKPI" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Leif Rydstedt" loading="lazy"> </div> <div class="info"> <hgroup><h1>Leif Rydstedt</h1> <h2>Professor Emeritus</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:leif.rydstedt@inn.no">leif.rydstedt@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4761288298"> +47 61 28 82 98 </a></dd> </dl> <p> Inland School of Business and Social Sciences<br> Department of Organisation, Leadership and Management<br> Campus Lillehammer </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.inn.no/english/find-an-employee/leif-rydstedt.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/leif-rydstedt.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=338923" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-MPJQXKPI" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg3" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg03_en.png" class="image" alt="SDG 3">
        <div class="sdg-overlay">
          <a href="{{< params subfolder >}}en/archive/?sdg=3#archive" class="sdg-publication-count"><span>1358</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal3" class="sdg-read-more">Read More</a></p>
        </div>
      </div> <div id="sdg8" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg08_en.png" class="image" alt="SDG 8">
        <div class="sdg-overlay">
          <a href="{{< params subfolder >}}en/archive/?sdg=8#archive" class="sdg-publication-count"><span>461</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal8" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-MPJQXKPI" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li><a href="{{< params subfolder >}}en/archive/?key=3DCRN523">University of Inland Norway</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=DU8Q9LN9">Inland School of Business and Social Sciences</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=KTD9NXA8">Department of Psychology</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=LS3MUAPD">2024</a></li>
      <li><a href="{{< params subfolder >}}en/archive/?key=WWLFELQK">April</a></li>
    </ul>
  </article>
</div>
