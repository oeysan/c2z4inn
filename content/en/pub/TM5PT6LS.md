---
title: '‘The Sound of Asking a Question’: Metalanguage and Crosslinguistic Awareness
  in Adults Learning Norwegian as an Additional Language'
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-TM5PT6LS" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Randen, G. T. (2022). ‘The Sound of Asking a Question’: Metalanguage and Crosslinguistic Awareness in Adults Learning Norwegian as an Additional Language. In M. Monsen &#38; G. B. Steien (Eds.), <i>Language learning and forced migration</i> (pp. 139–155). Multilingual Matters.</div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-TM5PT6LS" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2048287" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/TM5PT6LS" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-TM5PT6LS" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-TM5PT6LS" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-TM5PT6LS" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-TM5PT6LS" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
  </div>
  <div id="csl-bib-meta-container-TM5PT6LS"></div>
</article>
<div id="csl-bib-meta-TM5PT6LS" class="csl-bib-meta">
  <article id="about-article-TM5PT6LS" class="about_pub-article">
    <h1>About</h1>
    Randen conducted a study focusing on how refugees in Norway learn and use the Norwegian language. This research is important because it combines different perspectives—structural (how language is organized), cognitive (how language is processed in the brain), social (how language is used in society), and critical (how language relates to power and identity)—to understand language learning in a real-world context. By examining these aspects among the same group of individuals, the study provides a comprehensive view of how language is experienced by refugees. This can help improve language education and support for migrants, making it easier for them to integrate into new communities and navigate everyday life.
  </article>
  <article id="keywords-article-TM5PT6LS" class="keywords-article">
    <h1>Keywords</h1>
    Language Learning, Refugees, Norwegian, Crosslinguistic Awareness, Metalanguage
  </article>
  <article id="abstract-article-TM5PT6LS" class="abstract-article">
    <h1>Scientific abstract</h1>
    "This study of language issues in the context of migration provides interdisciplinary insights into language as learned, used and lived by refugees in Norway. It offers an innovative contribution to the field of SLA by bringing together structural, cognitive, social and critical approaches to data collected among the same individuals"
  </article>
  <article id="contributors-article-TM5PT6LS" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Gunhild Tveit Randen" loading="lazy"> </div> <div class="info"> <hgroup><h1>Gunhild Tveit Randen</h1> <h2>Associate Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:gunhild.randen@inn.no">gunhild.randen@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762517887"> +47 62 51 78 87 </a></dd> <dt>Mobile number</dt> <dd><a href="tel:+4740853629"> +47 40 85 36 29 </a></dd> </dl> <p> Faculty of education<br> Department of Scandinavian Languages and Literature<br> Campus Hamar, Room 2C256-4 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.79625,11.07386">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/gunhild-randen.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/gunhild-randen.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328387" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-TM5PT6LS" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg4" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg04_en.png" class="image" alt="SDG 4">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=4#archive" class="sdg-publication-count"><span>1352</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal4" class="sdg-read-more">Read More</a></p>
        </div>
      </div> <div id="sdg10" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg10_en.png" class="image" alt="SDG 10">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=10#archive" class="sdg-publication-count"><span>404</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal10" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-TM5PT6LS" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=WYNZA47F">Faculty of Education</a>
      </li>
      <li>
        <a href="/en/archive/?key=T9U6ILTU">Department of Scandinavian Languages and Literature</a>
      </li>
      <li>
        <a href="/en/archive/?key=8BZA2YRV">2022</a>
      </li>
      <li>
        <a href="/en/archive/?key=2JD3KQPG">September</a>
      </li>
    </ul>
  </article>
</div>
