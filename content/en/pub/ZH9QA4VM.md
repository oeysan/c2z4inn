---
title: 'Being second among the second: Experiences of Indigenous sports among assimilated
  Sámi'
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-ZH9QA4VM" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Skille, E., Fahlen, J., &#38; Lehtonen, K. (2025). Being second among the second: Experiences of Indigenous sports among assimilated Sámi. <i>International Review for the Sociology of Sport</i>, <i>60</i>(2), 322–340. <a href="https://doi.org/10.1177/10126902241268048">https://doi.org/10.1177/10126902241268048</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-ZH9QA4VM" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2291667" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/ZH9QA4VM" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-ZH9QA4VM" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-ZH9QA4VM" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-ZH9QA4VM" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-ZH9QA4VM" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://journals.sagepub.com/doi/pdf/10.1177/10126902241268048" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://journals.sagepub.com/doi/pdf/10.1177/10126902241268048" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-ZH9QA4VM"></div>
</article>
<div id="csl-bib-meta-ZH9QA4VM" class="csl-bib-meta">
  <article id="about-article-ZH9QA4VM" class="about_pub-article">
    <h1>About</h1>
    Skille et al. conducted a study on the dynamics within Indigenous Sámi sports clubs. They interviewed officials from clubs outside the core Sámi areas to understand their relationship with clubs in core Sámi regions and the broader Sámi sports organization. The findings reveal that officials from clubs outside core areas view their counterparts in core areas as superior, a perception that is deeply ingrained and accepted. This perceived hierarchy shows how power dynamics within the Sámi sports community can lead to further subordination of already marginalized subgroups. The study highlights the importance of recognizing and addressing these internal power struggles to ensure fair representation and influence within Indigenous communities.
  </article>
  <article id="keywords-article-ZH9QA4VM" class="keywords-article">
    <h1>Keywords</h1>
    Indigenous Community, Sámi Sports, Subordination, Symbolic Power, Sports Club Officials
  </article>
  <article id="abstract-article-ZH9QA4VM" class="abstract-article">
    <h1>Scientific abstract</h1>
    This article reports on a study investigating the struggle for influence in an Indigenous community. With an eye on the potential further subordination on certain subgroups, we studied how Sámi sports club officials outside Sámi core areas perceive their relationship with clubs in core Sámi areas and the federative Sámi sports organization. Methodologically, we performed interviews with representatives of Indigenous sports clubs and employed Bourdieu's concept of symbolic power as a theoretical framework. The results show how Sámi sports club officials outside core areas consider their peers within core Sámi areas as superior and that this relationship is engrained and taken for granted. The perceived superiority is based on the judgment of sports club officials outside the core Sámi areas, showing how the elite is defined as much by its subordinates as by the elite itself, to use Bourdieu's conceptualization of symbolic power. In conclusion, these results show how the struggle for influence in an Indigenous community can create further subordination of subgroups in a group that is already subordinated in society.
  </article>
  <article id="contributors-article-ZH9QA4VM" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Eivind Åsrum Skille" loading="lazy"> </div> <div class="info"> <hgroup><h1>Eivind Åsrum Skille</h1> <h2>Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:eivind.skille@inn.no">eivind.skille@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762430054"> +47 62 43 00 54 </a></dd> </dl> <p> Faculty of Social and Health Sciences<br> Section for Sports and Physical Education<br> Campus Elverum, Room 4R4142 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.88156,11.53723">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328312" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-ZH9QA4VM" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg16" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg16_en.png" class="image" alt="SDG 16">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=16#archive" class="sdg-publication-count"><span>479</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal16" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-ZH9QA4VM" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=IDKFS3MX">Faculty of Social and Health Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=FJXE3Z8X">Department of Public Health and Sport Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=DLUBDP8T">2024</a>
      </li>
      <li>
        <a href="/en/archive/?key=SGYLV4NK">September</a>
      </li>
    </ul>
  </article>
</div>
