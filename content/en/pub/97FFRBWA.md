---
title: Fra velferdsteknologi til «tingenes internett». Når velferd handler om samspillet
  mellom teknologier
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-97FFRBWA" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Hermanrud, I. (2024). Fra velferdsteknologi til «tingenes internett». Når velferd handler om samspillet mellom teknologier. <i>Tidsskrift for velferdsforskning</i>, <i>27</i>(4), 1–4. <a href="https://doi.org/10.18261/tfv.27.4.1">https://doi.org/10.18261/tfv.27.4.1</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-97FFRBWA" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2333172" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/97FFRBWA" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-97FFRBWA" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-97FFRBWA" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-97FFRBWA" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-97FFRBWA" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://doi.org/10.18261/tfv.27.4.1" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://doi.org/10.18261/tfv.27.4.1" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-97FFRBWA"></div>
</article>
<div id="csl-bib-meta-97FFRBWA" class="csl-bib-meta">
  <article id="about-article-97FFRBWA" class="about_pub-article">
    <h1>About</h1>
    Hermanrud's article explores the concept of the "Internet of Things" (IoT) and its impact on welfare services. IoT refers to the network of physical devices connected to the internet, enabling them to collect and exchange data. The author argues that IoT can significantly enhance welfare by mobilizing community resources to support health and care services, thereby easing the burden on local governments. The article outlines the conditions necessary for this development and concludes that welfare will increasingly depend on the interaction between private and municipal technologies. This shift could lead to more efficient and responsive welfare systems, benefiting both individuals and communities.
  </article>
  <article id="keywords-article-97FFRBWA" class="keywords-article">
    <h1>Keywords</h1>
    Welfare, Internet of Things, Municipalities, Health and Care, Civil Society Resources
  </article>
  <article id="abstract-article-97FFRBWA" class="abstract-article">
    <h1>Scientific abstract</h1>
    Denne artikkelen beskriver hva som menes med «tingenes internett». Gjennom å peke på konsekvenser for kommunene argumenterer jeg for teknologiens betydning for velferd. Ett argument jeg trekker frem, er at «tingenes internett» kan bidra til mobilisering av sivilsamfunnets ressurser for å avlaste kommunene innen helse og omsorg. Forutsetninger for en slik utvikling presenteres også. En hovedkonklusjon er at velferd i stor grad, høyst sannsynlig, kan komme til å handle mer og mer om samspillet og koblingen mellom privat og kommunal teknologi.
  </article>
  <article id="contributors-article-97FFRBWA" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Inge Hermanrud" loading="lazy"> </div> <div class="info"> <hgroup><h1>Inge Hermanrud</h1> <h2>Associate Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:inge.hermanrud@inn.no">inge.hermanrud@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4761288202"> +47 61 28 82 02 </a></dd> </dl> <p> Inland School of Business and Social Sciences<br> Department of Organisation, Leadership and Management<br> Campus Lillehammer </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.inn.no/english/find-an-employee/inge-hermanrud.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/inge-hermanrud.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328438" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-97FFRBWA" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg3" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg03_en.png" class="image" alt="SDG 3">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=3#archive" class="sdg-publication-count"><span>1358</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal3" class="sdg-read-more">Read More</a></p>
        </div>
      </div> <div id="sdg9" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg09_en.png" class="image" alt="SDG 9">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=9#archive" class="sdg-publication-count"><span>257</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal9" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-97FFRBWA" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=DU8Q9LN9">Inland School of Business and Social Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=4LUWR3ZM">Department of Organisation, Leadership and Management</a>
      </li>
      <li>
        <a href="/en/archive/?key=TY5PNNUR">2024</a>
      </li>
      <li>
        <a href="/en/archive/?key=YRAMKLSV">December</a>
      </li>
    </ul>
  </article>
</div>
