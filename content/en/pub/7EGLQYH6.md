---
title: 'Inside-out and outside-in: Applying the concept of conventions in the analysis
  of policy implementation through sport clubs'
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-7EGLQYH6" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Skille, E., &#38; Stenling, C. (2017). Inside-out and outside-in: Applying the concept of conventions in the analysis of policy implementation through sport clubs. <i>International Review for the Sociology of Sport</i>, <i>53</i>(7), 17–837. <a href="https://doi.org/10.1177/1012690216685584">https://doi.org/10.1177/1012690216685584</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-7EGLQYH6" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=1558324" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/7EGLQYH6" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-7EGLQYH6" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-7EGLQYH6" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-7EGLQYH6" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-7EGLQYH6" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://doi.org/10.1177/1012690216685584" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://doi.org/10.1177/1012690216685584" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-7EGLQYH6"></div>
</article>
<div id="csl-bib-meta-7EGLQYH6" class="csl-bib-meta">
  <article id="about-article-7EGLQYH6" class="about_pub-article">
    <h1>About</h1>
    Skille and Stenling examined how policies are implemented through sport clubs, highlighting a gap in current research. They found that existing studies focus too much on the internal workings of sport clubs and overlook the influence of the local community. Sport clubs are deeply rooted in their local areas, involving participants, volunteers, and relationships with other organizations. The authors argue that understanding policy implementation requires considering these external factors. They suggest using the concept of "convention," which looks at how outside perceptions and interactions affect the sport club's ability to implement policies. This approach can provide a more comprehensive understanding of how policies are put into practice in local sport clubs, potentially leading to more effective strategies.
  </article>
  <article id="keywords-article-7EGLQYH6" class="keywords-article">
    <h1>Keywords</h1>
    Policy Implementation, Sport Clubs, Local Community, Conventions, Outside-In Perspective
  </article>
  <article id="abstract-article-7EGLQYH6" class="abstract-article">
    <h1>Scientific abstract</h1>
    The aim of this article is to enrich theoretically the analysis of processes of policy implementation through sport clubs. Subsequent to reviewing previous theoretical contributions on this topic, we make the case that available conceptualizations are marked by an inside-out perspective and that they conceptually and empirically stop short at the end implementer, i.e. the sport club. Consequently, analyses of policy implementation through sport clubs have not taken into account the fact that sport clubs are distinctly local phenomena. As such, past, current and potential future participants, volunteers and local inter-organizational relationships are found in a sport club’s local community. Because of this, there is a need for a concept that provides analytical coverage of an outside-in perspective, i.e. a concept that takes into account outside actors’ conceptions of the implementing sport club and the impact their views have on the implementation process. In relation to this need, we propose the application of the concept ‘convention’. In addition to describing the concept, we exemplify the methodological and analytical implications of its use in sport policy implementation analysis.
  </article>
  <article id="contributors-article-7EGLQYH6" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Eivind Åsrum Skille" loading="lazy"> </div> <div class="info"> <hgroup><h1>Eivind Åsrum Skille</h1> <h2>Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:eivind.skille@inn.no">eivind.skille@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762430054"> +47 62 43 00 54 </a></dd> </dl> <p> Faculty of Social and Health Sciences<br> Section for Sports and Physical Education<br> Campus Elverum, Room 4R4142 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.88156,11.53723">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/eivind-skille.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328312" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-7EGLQYH6" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg11" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg11_en.png" class="image" alt="SDG 11">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=11#archive" class="sdg-publication-count"><span>232</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal11" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-7EGLQYH6" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=IDKFS3MX">Faculty of Social and Health Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=FJXE3Z8X">Department of Public Health and Sport Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=H5P87HVL">2018</a>
      </li>
      <li>
        <a href="/en/archive/?key=S2RACPMQ">January</a>
      </li>
    </ul>
  </article>
</div>
