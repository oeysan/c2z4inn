---
title: Norsk som andrespråk - perspektiver på læring og utvikling
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-SB7RSXA9" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Gujord, A.-K. H., &#38; Randen, G. T. (Eds.). (2018). <i>Norsk som andrespråk - perspektiver på læring og utvikling</i>. Cappelen Damm Akademisk. <a href="https://www.cappelendammundervisning.no/_norsk-som-andresprak-9788202584702">https://www.cappelendammundervisning.no/_norsk-som-andresprak-9788202584702</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-SB7RSXA9" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=1620654" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/SB7RSXA9" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-SB7RSXA9" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-SB7RSXA9" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-SB7RSXA9" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-SB7RSXA9" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
  </div>
  <div id="csl-bib-meta-container-SB7RSXA9"></div>
</article>
<div id="csl-bib-meta-SB7RSXA9" class="csl-bib-meta">
  <article id="about-article-SB7RSXA9" class="about_pub-article">
    <h1>About</h1>
    Gujord and Randen explore how people learn and develop Norwegian as a second language in Norway in their book. They discuss various methods of teaching that can support language learning. The authors present new insights into the process of acquiring Norwegian and compare Norwegian research with international studies. This book is useful for anyone interested in second language learning, including researchers, teachers of Norwegian, and students studying related subjects like multilingualism and language acquisition. Each chapter is written by experts in different areas of second language studies, making it a comprehensive resource for understanding how to effectively learn and teach Norwegian as a second language.
  </article>
  <article id="keywords-article-SB7RSXA9" class="keywords-article">
    <h1>Keywords</h1>
    Norwegian As A Second Language, Language Learning, Language Development, Multilingualism, Language Education
  </article>
  <article id="abstract-article-SB7RSXA9" class="abstract-article">
    <h1>Scientific abstract</h1>
    Hvordan lærer og utvikler man norsk som andrespråk i Norge?  
Hvordan kan undervisning støtte språklæringa? 
 
Denne boka dekker et bredt spekter av temaer i faget norsk som andrespråk. Forfatterne presenterer ny kunnskap om læring og utvikling av norsk som andrespråk, og plasserer den norske forskninga i et internasjonalt landskap. 
 
Boka passer for alle som er interessert i læring av andrespråk, for eksempel forskere, lærere i norsk og norsk som andrespråk og studenter i emner knyttet til norsk, norsk som andrespråk, flerspråklighet og språk generelt. 
 
Forfatterne av de ulike kapitlene er alle eksperter på forskjellige områder av andrespråksfeltet.
  </article>
  <article id="contributors-article-SB7RSXA9" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Gunhild Tveit Randen" loading="lazy"> </div> <div class="info"> <hgroup><h1>Gunhild Tveit Randen</h1> <h2>Associate Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:gunhild.randen@inn.no">gunhild.randen@inn.no</a> </dd> <dt>Phone number</dt> <dd><a href="tel:+4762517887"> +47 62 51 78 87 </a></dd> <dt>Mobile number</dt> <dd><a href="tel:+4740853629"> +47 40 85 36 29 </a></dd> </dl> <p> Faculty of education<br> Department of Scandinavian Languages and Literature<br> Campus Hamar, Room 2C256-4 </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.google.com/maps?q=60.79625,11.07386">Show on map</a></li> <li><a href="https://www.inn.no/english/find-an-employee/gunhild-randen.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/gunhild-randen.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328387" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-SB7RSXA9" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg4" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg04_en.png" class="image" alt="SDG 4">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=4#archive" class="sdg-publication-count"><span>1352</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal4" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-SB7RSXA9" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=WYNZA47F">Faculty of Education</a>
      </li>
      <li>
        <a href="/en/archive/?key=T9U6ILTU">Department of Scandinavian Languages and Literature</a>
      </li>
      <li>
        <a href="/en/archive/?key=6K7ZIYQV">2018</a>
      </li>
      <li>
        <a href="/en/archive/?key=CEAVH4LA">October</a>
      </li>
    </ul>
  </article>
</div>
