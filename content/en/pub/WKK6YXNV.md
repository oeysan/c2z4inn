---
title: 'Enabling and Constraining Conditions for Co-production with Vulnerable Users:
  A Case Study of Refugee Services'
type: pub
encoding: UTF-8

---
<h1>Publication</h1>
<article id="csl-bib-container-WKK6YXNV" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Røhnebæk, M., &#38; Bjerck, M. (2021). Enabling and Constraining Conditions for Co-production with Vulnerable Users: A Case Study of Refugee Services. <i>International Journal of Public Administration</i>, <i>44</i>(9), 741–752. <a href="https://doi.org/10.1080/01900692.2021.1908355">https://doi.org/10.1080/01900692.2021.1908355</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-WKK6YXNV" alt="archive" class="csl-bib-button">Archive</a>
    <a href="https://app.cristin.no/results/show.jsf?id=1902421" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/WKK6YXNV" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-WKK6YXNV" alt="keywords" class="csl-bib-button">Keywords</a>
    <a href="#about-article-WKK6YXNV" alt="about_pub" class="csl-bib-button">About</a>
    <a href="#contributors-article-WKK6YXNV" alt="contributors" class="csl-bib-button">Contributors</a>
    <a href="#sdg-article-WKK6YXNV" alt="sdg" class="csl-bib-button">Sustainable Development Goals</a>
    <a href="https://www.tandfonline.com/doi/pdf/10.1080/01900692.2021.1908355?needAccess=true" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://www.tandfonline.com/doi/pdf/10.1080/01900692.2021.1908355?needAccess=true" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-WKK6YXNV"></div>
</article>
<div id="csl-bib-meta-WKK6YXNV" class="csl-bib-meta">
  <article id="about-article-WKK6YXNV" class="about_pub-article">
    <h1>About</h1>
    Røhnebæk and Bjerck examined how public services can be co-produced with vulnerable users, specifically refugees. They identified three levels where co-production happens: direct interactions with services, within organizations, and across broader systems. They also distinguished between two types of co-production: direct participation, where users are actively involved, and representation, where users' needs are advocated by others. By studying refugee services, they developed a framework to analyze the conditions that enable or hinder co-production. This framework helps understand how to better involve vulnerable groups in shaping the services they receive, which can lead to more effective and responsive public services. Their findings are important for improving how public services engage with and support vulnerable populations.
  </article>
  <article id="keywords-article-WKK6YXNV" class="keywords-article">
    <h1>Keywords</h1>
    Co-production, Vulnerable Users, Refugee Services, Public Services, Service Interactions
  </article>
  <article id="abstract-article-WKK6YXNV" class="abstract-article">
    <h1>Scientific abstract</h1>
    This article explores co-production as processes occurring at three levels: at the level of service interactions, at the organizational level, and at the system level. We propose next that two forms of co-production may take place at these three levels: co-production through direct participation and co-production through representation. We use these conceptualizations to outline a framework for analyzing conditions for co-production with vulnerable users, and we explore the applicability of the framework in the context of public services for refugees. Insights from the case are finally used as a basis for evoking more general discussions on co-production with vulnerable users.
  </article>
  <article id="contributors-article-WKK6YXNV" class="contributors-article">
    <h1>Contributors</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Maria Røhnebæk" loading="lazy"> </div> <div class="info"> <hgroup><h1>Maria Røhnebæk</h1> <h2>Associate Professor</h2> </hgroup><dl> <dt>Email</dt> <dd> <a href="mailto:maria.rohnebak@inn.no">maria.rohnebak@inn.no</a> </dd> </dl> <p> Inland School of Business and Social Sciences<br> Department of Organisation, Leadership and Management<br> Campus Lillehammer </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.inn.no/english/find-an-employee/maria-rohnebak.html#vrtx-hinn-addresses">See Addresses</a></li> <li><a href="https://www.inn.no/english/find-an-employee/maria-rohnebak.html?vrtx=vcf">Download business card</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=312537" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-WKK6YXNV" class="sdg-article">
    <h1>Sustainable Development Goals</h1>
    <div class="sdg-container"><div id="sdg1" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg01_en.png" class="image" alt="SDG 1">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=1#archive" class="sdg-publication-count"><span>16</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal1" class="sdg-read-more">Read More</a></p>
        </div>
      </div> <div id="sdg10" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg10_en.png" class="image" alt="SDG 10">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=10#archive" class="sdg-publication-count"><span>404</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal10" class="sdg-read-more">Read More</a></p>
        </div>
      </div> <div id="sdg16" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg16_en.png" class="image" alt="SDG 16">
        <div class="sdg-overlay">
          <a href="/en/archive/?key=?sdg=16#archive" class="sdg-publication-count"><span>479</span> publications</a>
          <p><a href="https://sdgs.un.org/goals/goal16" class="sdg-read-more">Read More</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-WKK6YXNV" class="taxonomy-article">
    <h1>Archive</h1>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=DU8Q9LN9">Inland School of Business and Social Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=4LUWR3ZM">Department of Organisation, Leadership and Management</a>
      </li>
      <li>
        <a href="/en/archive/?key=8VQBC64H">2021</a>
      </li>
      <li>
        <a href="/en/archive/?key=6ZJVHCGB">April</a>
      </li>
    </ul>
    <ul>
      <li>
        <a href="/en/archive/?key=3DCRN523">University of Inland Norway</a>
      </li>
      <li>
        <a href="/en/archive/?key=DU8Q9LN9">Inland School of Business and Social Sciences</a>
      </li>
      <li>
        <a href="/en/archive/?key=IRYXBU4S">Eastern Norway Research Institute</a>
      </li>
      <li>
        <a href="/en/archive/?key=WVQ623KX">2021</a>
      </li>
      <li>
        <a href="/en/archive/?key=HTK8K4LC">April</a>
      </li>
    </ul>
  </article>
</div>
