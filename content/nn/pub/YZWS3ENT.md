---
title: 'Work, family life and recovery: An exploratory study of “the third shift”'
type: pub
encoding: UTF-8

---
<h1>Publikasjon</h1>
<article id="csl-bib-container-YZWS3ENT" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Ericsson, U., Pettersson, P., Rydstedt, L. W., &#38; Ekelund, E. (2021). Work, family life and recovery: An exploratory study of “the third shift”. <i>Work : A Journal of Prevention, Assesment and Rehabilitation</i>, <i>70</i>(4), 1131–1140. <a href="https://doi.org/10.3233/WOR-213624">https://doi.org/10.3233/WOR-213624</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-YZWS3ENT" alt="archive" class="csl-bib-button">Arkiv</a>
    <a href="https://app.cristin.no/results/show.jsf?id=1982508" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/YZWS3ENT" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-YZWS3ENT" alt="keywords" class="csl-bib-button">Emneord</a>
    <a href="#about-article-YZWS3ENT" alt="about_pub" class="csl-bib-button">Om</a>
    <a href="#contributors-article-YZWS3ENT" alt="contributors" class="csl-bib-button">Medverkande</a>
    <a href="#sdg-article-YZWS3ENT" alt="sdg" class="csl-bib-button">Berekraftsmål</a>
    <a href="https://content.iospress.com:443/download/work/wor213624?id=work%2Fwor213624" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://content.iospress.com:443/download/work/wor213624?id=work%2Fwor213624" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-YZWS3ENT"></div>
</article>
<div id="csl-bib-meta-YZWS3ENT" class="csl-bib-meta">
  <article id="about-article-YZWS3ENT" class="about_pub-article">
    <h1>Om</h1>
    Ericsson et al. gjennomførte ei utforskande studie for å forstå korleis folk restituere seg frå arbeid ved å undersøke deira daglege rutinar. Dei introduserte omgrepet "tredje skift," som refererer til tida brukt på å handtere familieansvar og fritidsaktivitetar etter arbeidstida. Ved å analysere 24-timars narrativ frå deltakarane, fann forfattarane at balansen mellom arbeid, familieliv og personleg tid er avgjerande for effektiv restitusjon. Denne studien framhevar viktigheita av å finne tid til avslapping og eigenomsorg midt i travle timeplanar. Å forstå denne balansen kan hjelpe individ med å forbetre sin generelle velvære og førebyggje utbrentheit, noko som gjer det relevant for alle som prøver å handtere sin arbeid-liv balanse.
  </article>
  <article id="keywords-article-YZWS3ENT" class="keywords-article">
    <h1>Emneord</h1>
    Arbeid, Familieforsiktingar, Fritid, Restitusjon, Tredje skift
  </article>
  <article id="abstract-article-YZWS3ENT" class="abstract-article">
    <h1>Samandrag</h1>
    Using 24-hour narratives as a starting-point, the present study examines conditions for recovery from work. 
The third shift concept forms the explorative starting point for highlighting the interplay between work, family responsibilities, leisure time and recovery
  </article>
  <article id="contributors-article-YZWS3ENT" class="contributors-article">
    <h1>Medverkande</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Leif Rydstedt" loading="lazy"> </div> <div class="info"> <hgroup><h1>Leif Rydstedt</h1> <h2>Professor emeritus</h2> </hgroup><dl> <dt>E-post</dt> <dd> <a href="mailto:leif.rydstedt@inn.no">leif.rydstedt@inn.no</a> </dd> <dt>Telefonnummer</dt> <dd><a href="tel:+4761288298"> +47 61 28 82 98 </a></dd> </dl> <p> Handelshøgskolen Innlandet – Fakultet for økonomi og samfunnsvitenskap<br> Institutt for organisasjon, ledelse, styring<br> Studiested Lillehammer </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.inn.no/finn-en-ansatt/leif-rydstedt.html#vrtx-hinn-addresses">Se adresser</a></li> <li><a href="https://www.inn.no/finn-en-ansatt/leif-rydstedt.html?vrtx=vcf">Last ned visittkort</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=338923" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-YZWS3ENT" class="sdg-article">
    <h1>Berekraftsmål</h1>
    <div class="sdg-container"><div id="sdg3" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg03_nn.png" class="image" alt="SDG 3">
        <div class="sdg-overlay">
          <a href="/nn/archive/?key=?sdg=3#archive" class="sdg-publication-count"><span>1358</span> publikasjonar</a>
          <p><a href="https://fn.no/om-fn/fns-baerekraftsmaal/god-helse-og-livskvalitet?lang=nno-NO" class="sdg-read-more">Les meir</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-YZWS3ENT" class="taxonomy-article">
    <h1>Arkiv</h1>
    <ul>
      <li>
        <a href="/nn/archive/?key=3DCRN523">Universitetet i Innlandet</a>
      </li>
      <li>
        <a href="/nn/archive/?key=DU8Q9LN9">Handelshøgskolen Innlandet - Fakultet for økonomi og samfunnsvitenskap</a>
      </li>
      <li>
        <a href="/nn/archive/?key=KTD9NXA8">Institutt for psykologi</a>
      </li>
      <li>
        <a href="/nn/archive/?key=AEVGZCNC">2022</a>
      </li>
      <li>
        <a href="/nn/archive/?key=64MCS3GE">Januar</a>
      </li>
    </ul>
  </article>
</div>
