---
title: Fra velferdsteknologi til «tingenes internett». Når velferd handler om samspillet
  mellom teknologier
type: pub
encoding: UTF-8

---
<h1>Publikasjon</h1>
<article id="csl-bib-container-97FFRBWA" class="csl-bib-container">
  <div class="csl-bib-body"> <div class="csl-entry">Hermanrud, I. (2024). Fra velferdsteknologi til «tingenes internett». Når velferd handler om samspillet mellom teknologier. <i>Tidsskrift for velferdsforskning</i>, <i>27</i>(4), 1–4. <a href="https://doi.org/10.18261/tfv.27.4.1">https://doi.org/10.18261/tfv.27.4.1</a></div> </div>
  <div class="csl-bib-buttons">
    <a href="#taxonomy-article-97FFRBWA" alt="archive" class="csl-bib-button">Arkiv</a>
    <a href="https://app.cristin.no/results/show.jsf?id=2333172" alt="Cristin" class="csl-bib-button">Cristin</a>
    <a href="http://zotero.org/groups/5881554/items/97FFRBWA" alt="Zotero" class="csl-bib-button">Zotero</a>
    <a href="#keywords-article-97FFRBWA" alt="keywords" class="csl-bib-button">Emneord</a>
    <a href="#about-article-97FFRBWA" alt="about_pub" class="csl-bib-button">Om</a>
    <a href="#contributors-article-97FFRBWA" alt="contributors" class="csl-bib-button">Medverkande</a>
    <a href="#sdg-article-97FFRBWA" alt="sdg" class="csl-bib-button">Berekraftsmål</a>
    <a href="https://doi.org/10.18261/tfv.27.4.1" alt="Unpaywall" class="csl-bib-button">Unpaywall</a>
    <a href="https://doi.org/10.18261/tfv.27.4.1" alt="EZproxy" class="csl-bib-button">EZproxy</a>
  </div>
  <div id="csl-bib-meta-container-97FFRBWA"></div>
</article>
<div id="csl-bib-meta-97FFRBWA" class="csl-bib-meta">
  <article id="about-article-97FFRBWA" class="about_pub-article">
    <h1>Om</h1>
    Hermanrud sin artikkel utforskar konseptet "Tingenes Internett" (IoT) og korleis det påverkar velferdstenester. IoT refererer til nettverket av fysiske einingar som er kopla til internett, og som gjer det mogleg å samle og utveksle data. Forfattaren argumenterer for at IoT kan betydeleg forbetre velferd ved å mobilisere samfunnsressursar til å støtte helse- og omsorgstenester, og dermed lette byrden på lokale styresmakter. Artikkelen skisserer dei nødvendige vilkåra for denne utviklinga og konkluderer med at velferd i aukande grad vil avhenge av samhandlinga mellom private og kommunale teknologiar. Denne endringa kan føre til meir effektive og responsive velferdssystem, som gagnar både individ og samfunn.
  </article>
  <article id="keywords-article-97FFRBWA" class="keywords-article">
    <h1>Emneord</h1>
    Velferd, Tingenes Internett, Teknologiintegrasjon, Sivilsamfunn, Helse og omsorg
  </article>
  <article id="abstract-article-97FFRBWA" class="abstract-article">
    <h1>Samandrag</h1>
    Denne artikkelen beskriver hva som menes med «tingenes internett». Gjennom å peke på konsekvenser for kommunene argumenterer jeg for teknologiens betydning for velferd. Ett argument jeg trekker frem, er at «tingenes internett» kan bidra til mobilisering av sivilsamfunnets ressurser for å avlaste kommunene innen helse og omsorg. Forutsetninger for en slik utvikling presenteres også. En hovedkonklusjon er at velferd i stor grad, høyst sannsynlig, kan komme til å handle mer og mer om samspillet og koblingen mellom privat og kommunal teknologi.
  </article>
  <article id="contributors-article-97FFRBWA" class="contributors-article">
    <h1>Medverkande</h1>
    <div class="personas"> <div class="vrtx-hinn-person-card"> <div class="photo"> <img src="data:image/jpeg;base64," alt="Inge Hermanrud" loading="lazy"> </div> <div class="info"> <hgroup><h1>Inge Hermanrud</h1> <h2>Førsteamanuensis</h2> </hgroup><dl> <dt>E-post</dt> <dd> <a href="mailto:inge.hermanrud@inn.no">inge.hermanrud@inn.no</a> </dd> <dt>Telefonnummer</dt> <dd><a href="tel:+4761288202"> +47 61 28 82 02 </a></dd> </dl> <p> Handelshøgskolen Innlandet – Fakultet for økonomi og samfunnsvitenskap<br> Institutt for organisasjon, ledelse, styring<br> Studiested Lillehammer </p> <ul class="vrtx-hinn-links"> <li><a href="https://www.inn.no/finn-en-ansatt/inge-hermanrud.html#vrtx-hinn-addresses">Se adresser</a></li> <li><a href="https://www.inn.no/finn-en-ansatt/inge-hermanrud.html?vrtx=vcf">Last ned visittkort</a></li> </ul> </div> </div> <a href="https://app.cristin.no/persons/show.jsf?id=328438" alt="Cristin URL" class="personas-cristin">Cristin</a> </div>
  </article>
  <article id="sdg-article-97FFRBWA" class="sdg-article">
    <h1>Berekraftsmål</h1>
    <div class="sdg-container"><div id="sdg3" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg03_nn.png" class="image" alt="SDG 3">
        <div class="sdg-overlay">
          <a href="/nn/archive/?key=?sdg=3#archive" class="sdg-publication-count"><span>1358</span> publikasjonar</a>
          <p><a href="https://fn.no/om-fn/fns-baerekraftsmaal/god-helse-og-livskvalitet?lang=nno-NO" class="sdg-read-more">Les meir</a></p>
        </div>
      </div> <div id="sdg9" class="sdg">
        <img src="{{< params subfolder >}}images/sdg/sdg09_nn.png" class="image" alt="SDG 9">
        <div class="sdg-overlay">
          <a href="/nn/archive/?key=?sdg=9#archive" class="sdg-publication-count"><span>257</span> publikasjonar</a>
          <p><a href="https://fn.no/om-fn/fns-baerekraftsmaal/industri-innovasjon-og-infrastruktur?lang=nno-NO" class="sdg-read-more">Les meir</a></p>
        </div>
      </div></div>
  </article>
  <article id="taxonomy-article-97FFRBWA" class="taxonomy-article">
    <h1>Arkiv</h1>
    <ul>
      <li>
        <a href="/nn/archive/?key=3DCRN523">Universitetet i Innlandet</a>
      </li>
      <li>
        <a href="/nn/archive/?key=DU8Q9LN9">Handelshøgskolen Innlandet - Fakultet for økonomi og samfunnsvitenskap</a>
      </li>
      <li>
        <a href="/nn/archive/?key=4LUWR3ZM">Institutt for organisasjon, ledelse, styring</a>
      </li>
      <li>
        <a href="/nn/archive/?key=TY5PNNUR">2024</a>
      </li>
      <li>
        <a href="/nn/archive/?key=YRAMKLSV">Desember</a>
      </li>
    </ul>
  </article>
</div>
